...
export default function HomePage() {
  ...
  return (
    <main style={{ background: "#0a0406", fontFamily: "'Georgia', 'Times New Roman', serif", overflowX: "hidden", minHeight: "100vh" }}>
      ...
      {/* Nav */}
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "1.25rem 3rem", display: "flex", alignItems: "center", justifyContent: "space-between", background: navSolid ? "rgba(10,4,6,0.95)" : "rgba(10,4,6,0.6)", backdropFilter: "blur(12px)", borderBottom: navSolid ? "1px solid rgba(200,16,46,0.15)" : "1px solid transparent", transition: "all 0.3s" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "2rem" }}>
          {[
            ...
            <button key={tab.id} onClick={() => scrollTo(tab.id)} style={{ background: "transparent", border: "none", cursor: "pointer", color: "rgba(245,240,235,0.5)", fontSize: "0.8rem", letterSpacing: "0.1em", textTransform: "uppercase", fontFamily: "'Georgia', serif", transition: "color 0.2s" }}
      ...
      {/* Fixed CD */}
      <div style={{ position: "fixed", right: "-8vw", top: "50%", width: "55vw", aspectRatio: "1", zIndex: 5, transform: "translateY(-50%)", pointerEvents: "none" }}>
        <div style={{ width: "100%", height: "100%", borderRadius: "50%", position: "relative", transform: `rotate(${cdRotation}deg)`, willChange: "transform" }}>
          <div style={{ position: "absolute", inset: 0, borderRadius: "50%", background: `conic-gradient(from 0deg, #1c0810 0deg, #2e1018 30deg, #150608 70deg, #3a1020 110deg, #150608 150deg, #2a0e16 190deg, #150608 230deg, #3a1020 270deg, #150608 310deg, #1c0810 360deg)`, boxShadow: "inset 0 0 80px rgba(0,0,0,0.7), 0 0 120px rgba(139,0,20,0.2)" }} />
      ...
      <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", padding: "8rem 3rem 4rem", position: "relative", zIndex: 10 }}>
        <div style={{ maxWidth: "520px", opacity: mounted ? 1 : 0, transform: mounted ? "translateY(0)" : "translateY(20px)", transition: "opacity 0.8s ease, transform 0.8s ease" }}>
      ...
      <section id="how-it-works" style={{ padding: "8rem 3rem", position: "relative", zIndex: 10, borderTop: "1px solid rgba(139,0,20,0.15)" }}>
        <p className="reveal" style={{ fontSize: "0.7rem", letterSpacing: "0.35em", textTransform: "uppercase", color: "#c8102e", marginBottom: "1rem", textAlign: "center" }}>How It Works</p>
        <h2 className="reveal reveal-delay-1" style={{ fontSize: "clamp(2rem, 4vw, 3.5rem)", fontWeight: "700", color: "#f5f0eb", textAlign: "center", marginBottom: "5rem", letterSpacing: "-0.02em" }}>Three steps to viral.</h2>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", maxWidth: "900px", margin: "0 auto", border: "1px solid rgba(139,0,20,0.15)" }}>
          {[
            { num: "01", title: "Upload Your Song", desc: "Drop an MP3 or MP4. We accept any format.", delay: "reveal-delay-2", bg: "rgba(139,0,20,0.06)" },
            { num: "02", title: "AI Does Everything", desc: "Beat detection, mood analysis, clip selection, lyric sync. All automatic.", delay: "reveal-delay-3", bg: "rgba(100,0,30,0.08)" },
            { num: "03", title: "Download & Post", desc: "Get a 4K video ready for Reels, TikTok, and YouTube Shorts.", delay: "reveal-delay-4", bg: "rgba(60,0,40,0.1)" },
          ].map((f, i) => (
            <div key={i} className={`reveal ${f.delay}`} style={{ padding: "3rem 2rem", borderRight: i < 2 ? "1px solid rgba(139,0,20,0.15)" : "none", background: f.bg, transition: "background 0.3s, opacity 0.8s ease, transform 0.8s ease" }}
              onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(139,0,20,0.15)")}
              onMouseLeave={(e) => (e.currentTarget.style.background = f.bg)}
            >
              <p style={{ fontSize: "3rem", fontWeight: "700", color: "rgba(139,0,20,0.25)", marginBottom: "1.5rem", letterSpacing: "-0.03em" }}>{f.num}</p>
      ...
      <section id="features" style={{ padding: "8rem 3rem", borderTop: "1px solid rgba(139,0,20,0.15)", position: "relative", zIndex: 10 }}>
        <p className="reveal" style={{ fontSize: "0.7rem", letterSpacing: "0.35em", textTransform: "uppercase", color: "#c8102e", marginBottom: "1rem", textAlign: "center" }}>Features</p>
        <h2 className="reveal reveal-delay-1" style={{ fontSize: "clamp(2rem, 4vw, 3.5rem)", fontWeight: "700", color: "#f5f0eb", textAlign: "center", marginBottom: "5rem", letterSpacing: "-0.02em" }}>Everything you need.</h2>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "1px", maxWidth: "1000px", margin: "0 auto", background: "rgba(139,0,20,0.08)", border: "1px solid rgba(139,0,20,0.08)" }}>
          {[
            { title: "AI Beat Detection", desc: "Every cut lands on a beat. Every time. Automatic.", delay: "reveal-delay-1", bg: "rgba(139,0,20,0.06)" },
            { title: "Word-by-Word Captions", desc: "Real lyrics synced to the millisecond. 5 caption styles.", delay: "reveal-delay-2", bg: "rgba(110,0,25,0.07)" },
            { title: "6 Templates", desc: "Proven viral formats. More added every month for Business users.", delay: "reveal-delay-3", bg: "rgba(80,0,35,0.08)" },
            { title: "4K Export", desc: "Download at full resolution ready to post anywhere.", delay: "reveal-delay-4", bg: "rgba(60,0,40,0.09)" },
            { title: "Mood Matching", desc: "AI picks clips that match your song's energy and vibe.", delay: "reveal-delay-2", bg: "rgba(100,0,30,0.07)" },
            { title: "No Editing Required", desc: "Upload. Pick a template. Download. Done.", delay: "reveal-delay-3", bg: "rgba(139,0,20,0.06)" },
          ].map((f, i) => (
            <div key={i} className={`reveal ${f.delay}`} style={{ padding: "2.5rem 2rem", background: f.bg, transition: "background 0.2s, opacity 0.8s ease, transform 0.8s ease" }}
              onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(139,0,20,0.15)")}
              onMouseLeave={(e) => (e.currentTarget.style.background = f.bg)}
            >
      ...
      <section id="pricing" style={{ padding: "8rem 3rem", borderTop: "1px solid rgba(139,0,20,0.15)", position: "relative", zIndex: 10 }}>
        <p className="reveal" style={{ fontSize: "0.7rem", letterSpacing: "0.35em", textTransform: "uppercase", color: "#c8102e", marginBottom: "1rem", textAlign: "center" }}>Pricing</p>
        <h2 className="reveal reveal-delay-1" style={{ fontSize: "clamp(2rem, 4vw, 3.5rem)", fontWeight: "700", color: "#f5f0eb", textAlign: "center", marginBottom: "1rem", letterSpacing: "-0.02em" }}>Simple. No surprises.</h2>
        <p className="reveal reveal-delay-2" style={{ color: "rgba(245,240,235,0.35)", textAlign: "center", marginBottom: "5rem", fontSize: "0.95rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "1px", maxWidth: "900px", margin: "0 auto", background: "rgba(139,0,20,0.08)" }}>
          {[
            { name: "Starter", price: "$9", features: ["10 videos/month", "No watermark", "1080p export", "3 templates"], featured: false, delay: "reveal-delay-2", bg: "rgba(139,0,20,0.05)" },
            { name: "Pro", price: "$19", features: ["Unlimited videos", "No watermark", "1080p export", "All templates", "Priority support"], featured: true, delay: "reveal-delay-3", bg: "rgba(139,0,20,0.15)" },
            { name: "Business", price: "$49", features: ["Unlimited videos", "No watermark", "4K export", "All templates", "New templates monthly"], featured: false, delay: "reveal-delay-4", bg: "rgba(80,0,40,0.08)" },
          ].map((plan, i) => (
